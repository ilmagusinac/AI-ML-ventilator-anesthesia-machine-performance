...
- Trains Gradient Boosting model with GridSearchCV
...
calibrated_gb = CalibratedClassifierCV(grid_search.best_estimator_, cv=3)# Make probability outputs more accurate and realistic
calibrated_gb.fit(X_train, y_train)

# Get trustworthy probabilities for risk assessment
y_proba = calibrated_gb.predict_proba(X_test)[:, 1]
y_pred = calibrated_gb.predict(X_test) # Classify passed/failed when needed
...
probas = calibrated_gb.predict_proba(X)
