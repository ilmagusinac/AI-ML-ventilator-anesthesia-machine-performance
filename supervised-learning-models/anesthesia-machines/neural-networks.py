...
- Trains Naive Bayes model with MLPClassifier
...
#ata = pd.read_csv("files/anesthesia_machines_2015_2023.csv", encoding="utf-8")
...
# Scale features (KNN is distance-based and very sensitive to scale)
...
mlp = MLPClassifier(
    hidden_layer_sizes=(100,),   # you can tune this
    activation='relu',           # or 'tanh', 'logistic'
    solver='adam',               # or 'sgd'
    max_iter=500,                # increase if convergence warning appears
    random_state=42
)
mlp.fit(X_train_scaled, y_train)  # Training the model
...
calibrated_mlp = CalibratedClassifierCV(mlp, cv=3)
# Make probability outputs more accurate and realistic
calibrated_mlp.fit(X_train_scaled, y_train)

# Get trustworthy probabilities for risk assessment
y_pred = calibrated_mlp.predict(X_test_scaled)  # Classify passed/failed when needed
y_proba = calibrated_mlp.predict_proba(X_test_scaled)[:, 1]
...
probas = calibrated_mlp.predict_proba(X_scaled_all)
...
X_next_year_scaled = pd.DataFrame(scaler.transform(X_next_year), columns=X_next_year.columns, index=X_next_year.index)

probas_next_year = calibrated_mlp.predict_proba(X_next_year_scaled)
