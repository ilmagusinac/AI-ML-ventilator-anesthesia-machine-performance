...
# Optional: Calibrate MLP for better probability estimates
calibrated_mlp = CalibratedClassifierCV(mlp, cv=3)
calibrated_mlp.fit(X_train_scaled, y_train)

# Predict
y_pred = calibrated_mlp.predict(X_test_scaled)
y_proba = calibrated_mlp.predict_proba(X_test_scaled)[:, 0]  # probability for class 0 (NE)
