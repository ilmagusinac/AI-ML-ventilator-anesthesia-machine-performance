...
#gnb = GaussianNB()
#class_prior = [0.5, 0.5]  # force balance (or try actual ratios if preferred)
#gnb = GaussianNB(priors=class_prior)
gnb = GaussianNB()
gnb.fit(X_train_scaled, y_train)

calibrated_gnb = CalibratedClassifierCV(gnb, cv=3)
calibrated_gnb.fit(X_train_scaled, y_train)

# Predictions
y_pred = calibrated_gnb.predict(X_test_scaled)
y_proba = calibrated_gnb.predict_proba(X_test_scaled)[:, 0] 
