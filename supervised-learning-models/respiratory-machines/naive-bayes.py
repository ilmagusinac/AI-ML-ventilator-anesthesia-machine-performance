...
- Trains Naive Bayes model with GaussianNB
...
y_pred = calibrated_gnb.predict(X_test_scaled) # Classify passed/failed when needed
y_proba = calibrated_gnb.predict_proba(X_test_scaled)[:, 1] 
...
X_scaled_all = X
probas = calibrated_gnb.predict_proba(X_scaled_all)
